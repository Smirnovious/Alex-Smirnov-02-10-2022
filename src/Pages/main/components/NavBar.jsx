  ...
  return (
    <div className='text-2xl flex justify-between dark:bg-gray-800'>
      ...
      <div className='flex flex-row items-center'>
          ...
          <Link to={'/favorites'}><button className='w-10 h-10 mr-2 dark:text-red-500'><ImHeart/></button></Link>  
            <span className="absolute right-28 top-5 flex justify-center 
                      items-center w-4 h-4 text-xs font-bold text-black 
                      dark:text-white bg-white  dark:bg-yellow-500 rounded-full border- 
                      border-white">0
                      </span>
          <ToggleDarkMode/>
          <button onClick={()=> dispatch(toggleTemp())} className='w-10 h-10 mr-2 dark:text-amber-200'>{isMetric ? '\xB0C' : '\xB0F'}</button>
      </div>
    </div>
  )
}
